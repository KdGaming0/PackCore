...
- Setup pages that configure a specific mod (item backgrounds, storage design, tab design, dungeon routes, sword block, ScamScreener, Caxton fonts) are now hidden when that mod is not installed, so the wizard only shows options that actually work for your setup.
- The storage design option now integrates with Enhanced Storage instead of Firmament.
- The dungeon routes setup page is back and now lets you choose between Skyblocker Waypoints (recommended) and Stella Secret Routes.